...
namespace N4Core.Accounts.Services.Bases
{
    public abstract class AccountServiceBase : IDisposable
    {
        ...
        protected readonly RepoBase<AccountUser> _repo;
        protected readonly AccountUtilBase _accountUtil;
        protected readonly CultureUtilBase _cultureUtil;

        protected AccountServiceBase(UnitOfWorkBase unitOfWork, RepoBase<AccountUser> repo, AccountUtilBase accountUtil, CultureUtilBase cultureUtil)
        {
        ...
        {
            var existingUser = await _repo.Query().Include(q => q.Role).SingleOrDefaultAsync(q => q.UserName == userName && q.Password == password && q.IsActive, cancellationToken);
        ...
        {
            if (await _repo.Query().AnyAsync(q => q.UserName == model.UserName.Trim(), cancellationToken))
