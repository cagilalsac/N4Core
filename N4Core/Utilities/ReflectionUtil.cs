...
namespace N4Core.Utilities
{
    public class ReflectionUtil : IReflectionUtil
	{
		...
		public List<ReflectionPropertyModel> GetReflectionPropertyModelProperties<T>(TagAttribute tagAttribute = TagAttribute.None) where T : class
		{
		...
		public Expression<Func<T, bool>> GetPredicateContainsExpression<T>(string propertyName, string value, bool isCaseInsensitive = true) where T : class, new()
        {
			...
			if (isCaseInsensitive)
			{
				...
				containsCall = Expression.Call(Expression.Call(property, toUpperMethod), containsMethod, Expression.Constant(value ?? ""));
			}
			else
			{
				containsCall = Expression.Call(property, containsMethod, Expression.Constant(value ?? ""));
			}
			return Expression.Lambda<Func<T, bool>>(containsCall, parameter);
		}

		public Expression<Func<T, object>> GetExpression<T>(string propertyName) where T : class, new()
        {
			var parameter = Expression.Parameter(typeof(T), "t");
		...
		public IQueryable<T> OrderQuery<T>(IQueryable<T> query, bool orderDirectionDescending, string orderExpression, string orderExpressionSuffix) where T : class, new()
        {
		...
		public ReflectionRecordModel GetReflectionRecordModel<T>() where T : class, new()
		{
			...
			property = GetReflectionPropertyModelProperties<IRecordFile>()?[3];
